...
public class JointOrientationPrimary : MonoBehaviour
{
    ...
    void Update ()
    {
		...
		if (relativeRoll - _prevRoll > 5) {
			//ExtendUnlockAndNotifyUserAction (thalmicMyo);
			if (audioSource.volume >= .45f) {

				audioSource.volume = audioSource.volume - volumeStep;


			} 
		} else if (relativeRoll - _prevRoll < -5) {
