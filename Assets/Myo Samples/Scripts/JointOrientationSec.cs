...
public class JointOrientationSec : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (relativeRoll - _prevRoll > 50) {
			//ExtendUnlockAndNotifyUserAction (thalmicMyo);
			if (audioSource.pitch >= -3f) {

				audioSource.pitch = audioSource.pitch - pitchStep;


			} 
		} else if (relativeRoll - _prevRoll < -50) {
			if (audioSource.pitch <= 3f) {
